import { EDITOR_REGIONS } from "../../editorFocus/useEditorFocusStore";
import type { ExtensionApiScope, ExtensionDisposable } from "../types";
...
/**
 * The single chord table behind command dispatch. Host defaults and extension
 * requests share it; a binding that collides with an earlier active binding on
 * the same platform-resolved chord (with overlapping regions) registers as
 * inactive with a diagnostic, so a chord never has two owners at dispatch and
 * an extension collision never fails activation.
 */
export class HostKeybindingRegistry {
   ...
   * so an extension binding on a reserved chord registers inactive with a
  ...
  registerExtensionBinding(
    ...
    assertBindingShape(binding);
    return this.add({
      id: `${scope.extension.id}/${binding.id}`,
      source: "extension",
      chord: parseChord(binding.chord),
      chordLabel: binding.chord,
      commandId: binding.commandId,
  ...
  dispatch(
    ...
    for (const entry of this.entries) {
      // Reservations block extension bindings at registration; the inline
  ...
  /**
   * Host bindings shadow extension bindings; earlier registrations shadow
  ...
  private recomputeActivation(): void {
    ...
    for (const entry of ordered) {
      ...
      if (!nextActive && entry.active && entry.report) {
        entry.report(
          "warning",
