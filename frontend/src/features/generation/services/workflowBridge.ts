 ...
 * (`services/workflow_rules/object_info.py`). See `graphSubgraphs.ts` for the
 * boundary and promoted-widget rules that walk follows.
 *
...
export function parseInputsFromGraphData(
  ...
  for (const flat of flattenGraphNodes(graphData)) {
    const { node, nodeId, classType, linkedParams, promotedValues } = flat;
    if (flat.muted || !classType) continue;
    ...
    for (const mapping of mappings) {
      ...
      if (!linkedParams.has(mapping.param)) {
        const promoted = promotedValues.get(mapping.param);
        const widgetIndex = widgetIndexMap.get(mapping.param);
        if (promoted !== undefined) {
          // A promoted widget's value lives on the subgraph instance node and
          // is what executes, even when the inner node still carries a
          // different (stale) value of its own.
          currentValue = promoted;
        } else if (
          widgetIndex !== undefined &&
          widgetIndex < widgetsValues.length
        ) {
