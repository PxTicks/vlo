 ...
 * the inputs were invalid (depth < 1, non-positive duration, negative
 * start) — same no-op + warn shape as the v1 group helpers.
 *
...
export function createAdjustmentClipInDraft(
  ...
  if (input.depth < 1) {
    console.warn(
      `[createAdjustmentClipInDraft] rejecting clip: depth must be ≥ 1 (got ${input.depth}).`,
    );
    return null;
  }
 ...
 * doesn't exist, isn't an adjustment clip, or `depth < 1`. Depth bounds
 * against available tracks below are NOT enforced here — the derivation
 * clamps at render time, so a depth that exceeds the current track stack
 * is harmless and restores reach if the user re-adds tracks below.
 */
