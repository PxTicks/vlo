...
describe("useTransformInteractionController", () => {
  ...
  it("materializes and commits a scale transform from a corner handle", () => {
    const clip = {
      id: "clip_scale",
      trackId: "track_1",
      type: "video",
    ...
    useTimelineStore.getState().addClip(clip);
    activeClipRef = { current: clip };
    const liveTransform = vi.fn();
    const notify = vi.spyOn(liveParamStore, "notify");
    const { result } = renderHook(() =>
      useTransformInteractionController(
        mockSprite,
        activeClipRef,
    ...
    expect(liveTransform).toHaveBeenCalled();
  ...
  it("materializes and commits rotation from rotate and alt-drag handles", () => {
    ...
    useTimelineStore.getState().addClip(clip);
    activeClipRef = { current: clip };
    mockSprite.getGlobalPosition = vi.fn(() => ({ x: 0, y: 0 })) as never;
    const notify = vi.spyOn(liveParamStore, "notify");
    const { result } = renderHook(() =>
      useTransformInteractionController(
        mockSprite,
    ...
    const rotation = activeClipRef.current?.transformations.find(
    ...
    activeClipRef.current = useTimelineStore
      .getState()
      .clips.find((candidate) => candidate.id === clip.id) ?? null;
    act(() => {
      result.current.onHandlePointerDown(
        {
          stopPropagation: vi.fn(),
    ...
    expect(mockApp.stage.on).toHaveBeenCalledWith(
      "pointermove",
      expect.any(Function),
    );
    notify.mockRestore();
  ...
  it("ignores invalid pointer starts and clips with inactive persisted paths", () => {
    activeClipRef = { current: null };
    const { result, rerender } = renderHook(
      ({ sprite, viewport }: { sprite: Sprite | null; viewport: Container | null }) =>
        useTransformInteractionController(
          sprite,
          activeClipRef,
          mockApp,
      ...
      { initialProps: { sprite: mockSprite, viewport: mockViewport } },
    );
    ...
    act(() => {
      ...
      result.current.onHandlePointerDown(
        { global: { x: 0, y: 0 } } as unknown as FederatedPointerEvent,
        "se",
      );
    });
    expect(mockApp.stage.on).not.toHaveBeenCalled();

    const clip = {
      id: "clip_path_inactive",
      trackId: "track_1",
      type: "video",
      assetId: "asset_1",
      start: 0,
    ...
    activeClipRef.current = clip;
    rerender({ sprite: mockSprite, viewport: mockViewport });
    act(() => {
      result.current.onSpritePointerDown({
        button: 1,
        global: { x: 0, y: 0 },
  ...
  it("hides path overlays for unselected, invisible, or pathless clips", () => {
    const clip = {
      id: "clip_overlay_hidden",
      trackId: "track_1",
      type: "video",
      assetId: "asset_1",
      start: 0,
    ...
    useTimelineStore.setState({ clips: [clip], selectedClipIds: [] });
    activeClipRef = { current: clip };
    ...
    act(() => {
      useTimelineStore.setState({ selectedClipIds: [clip.id] });
      drawOverlay?.();
    });
    unmount();
