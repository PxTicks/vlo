...
function normalizeRenderMode(
  mode:
    | DerivedMaskMapping["sourceSelection"]
    | DerivedMaskMapping["maskSelection"],
...
/**
 * Prepared selection renders depend on the workflow's source/mask semantics.
 * The signature prevents a filtered source render from being reused after the
 * workflow changes to a full-selection source (or vice versa).
 */
