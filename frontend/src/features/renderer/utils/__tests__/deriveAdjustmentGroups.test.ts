...
describe("computeAdjustmentApplications", () => {
  ...
  it('the "all" sentinel grows with newly added lower tracks', () => {
    const initialTracks = [adjustmentTrack("adj"), visualTrack("v1")];
    ...
    const initialApps = computeAdjustmentApplications(initialTracks, clips, 50);
    expect(initialApps.get("v1")).toHaveLength(1);
    expect(initialApps.get("v2")).toBeUndefined();

    const expandedApps = computeAdjustmentApplications(expandedTracks, clips, 50);
    expect(expandedApps.get("v1")).toHaveLength(1);
    expect(expandedApps.get("v2")).toHaveLength(1);
  });
