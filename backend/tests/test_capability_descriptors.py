...
def test_descriptor_identities_are_unique() -> None:
    ...
    # Same for the environment snapshot's ``searchPaths``.
    snapshot_keys = [descriptor.snapshot_key for descriptor in table]
...
def _fake_module(*, outcome: str = "ok") -> ModuleType:
    ...
    def build(on_progress=None):
        ...
        return RuntimeLoad(value=object(), resolved_device="cuda")
...
def _fake_descriptor(
    *,
    ...
    return CapabilityDescriptor(
        ...
        python_min=(3, 10),
        device_env_var="VLO_FAKE_DEVICE",
        cache_dirs=(
            DirectorySpec(
                id="vloFake.cache",
                config_attr="VLO_FAKE_CACHE_DIR",
                label="The fake cache directory",
            ),
        ),
        search_paths=("VLO_FAKE_SEARCH_PATHS",),
...
def fake_capability(
    ...
    monkeypatch.setattr(config, "VLO_FAKE_CACHE_DIR", cache_dir, raising=False)
    monkeypatch.setattr(
        config, "VLO_FAKE_SEARCH_PATHS", [models_dir], raising=False
    )
    ...
    try:
        ...
        # Unregistering drops the cell, the recorded failure and the load
        # observation with it, so nothing survives into the next test.
...
def test_unregistering_forgets_what_the_old_runtime_established(
    ...
    unregister_descriptor(FAKE_ID)
    register_descriptor(_fake_descriptor())
...
def test_a_successful_load_through_the_cell_proves_the_loaded_stage(
    ...
    assert capability.device.resolved == "cuda"
